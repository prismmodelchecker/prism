...
public class StochModel implements Model
{
	// model info
	
	// type
	private static final String type = "Stochastic (CTMC)";
	// modules
	...
	// dds
	private JDDNode trans;			// transition rate matrix dd
	...
	public StochModel(JDDNode tr, JDDNode tr01, JDDNode s, JDDNode r, JDDNode dl, JDDNode sr, JDDNode trr, JDDVars arv, JDDVars acv, Vector ddvn,
...
//		JDD.PrintMatrix(trans01, allDDRowVars, allDDColVars, JDD.ZERO_ONE);
	...
	{
		if (!explicit) {
			PrismMTBDD.ExportMatrix(trans, "R", allDDRowVars, allDDColVars, odd, exportType, (file != null)?file.getPath():null);
		}
		else {
			PrismSparse.ExportMatrix(trans, "R", allDDRowVars, allDDColVars, odd, exportType, (file != null)?file.getPath():null);
		}
	}
	...
	public void exportTransRewardsToFile(int exportType, boolean explicit, File file) throws FileNotFoundException, PrismException
	{
		...
		if (!explicit) {
			PrismMTBDD.ExportMatrix(transRewards, "C", allDDRowVars, allDDColVars, odd, exportType, (file != null)?file.getPath():null);
		}
		else {
			PrismSparse.ExportMatrix(transRewards, "C", allDDRowVars, allDDColVars, odd, exportType, (file != null)?file.getPath():null);
		}
	}
