...
public class ExpressionTimes extends ExpressionBinary
{
	...
	public int toSimulator(SimulatorEngine sim) throws SimulatorException
	{
		...
		{
			throw new SimulatorException("Error when creating times operator:\n"+e.getMessage());
		}
		
		if(theType == Expression.DOUBLE)
			return SimulatorEngine.createRealTimes(operand1.toSimulator(sim), operand2.toSimulator(sim));
		else
			return SimulatorEngine.createNormalTimes(operand1.toSimulator(sim), operand2.toSimulator(sim));
	}
