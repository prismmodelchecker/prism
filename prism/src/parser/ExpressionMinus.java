...
public class ExpressionMinus extends ExpressionBinary
{
	...
	{  
	    return apmc.newBinaryOperand(apmc.MINUS, operand1.toApmc(apmc), operand2.toApmc(apmc));
	}
	...
	public int toSimulator(SimulatorEngine sim) throws SimulatorException
	{
		...
		{
			throw new SimulatorException("Error when creating minus operator:\n"+e.getMessage());
		}
		
		if(theType == Expression.DOUBLE)
			return SimulatorEngine.createRealMinus(operand1.toSimulator(sim), operand2.toSimulator(sim));
		else
			return SimulatorEngine.createNormalMinus(operand1.toSimulator(sim), operand2.toSimulator(sim));
	}
