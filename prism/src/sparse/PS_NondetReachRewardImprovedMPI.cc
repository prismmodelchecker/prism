...
JNIEXPORT jlong __jlongpointer JNICALL Java_sparse_PrismSparse_PS_1NondetReachRewardImprovedMPI
(
JNIEnv *env,
jclass cls,
...
)
{
	...
	int *dns_row_starts2 = new int[n + 1];
	int *dns_cols2 = new int[numTransitions];
	int *dns_choice_starts_r2 = new int[row_starts[n] + 50];
	//int *dns_cols_r2 = new int[numTransitions];
	double *dns_sr_vec2 = new double[n];
	double *dns_nnz2 = new double[numTransitions];
	double *dns_nnz_r2 = new double[numTransitions];
	for(i = 0; i < n; i++)
		selected[i] = -1;
	int _ind = 0;
	l1 = 1;
	...
	double diff, d3, d4, diff2;	
	int ind;
	iters = 0;
	int m1, m2, m3, m_r, j2, k2;
	dns_row_starts[0] = 0;
	m1 = m2 = m3 = m_r = 0;
	...
	{	tt = 0;
		iters++;
		if(iters % 500 <= 250)printf(" %d ", iters);
		diff = 0;	
		// do matrix multiplication and min/max (from below)
		h1 = h2 = h2_r = 0;
		j2 = k2 = 0;
		// loop through states
		dns_row_starts2[0] = 0;	// added for MPI.
		for (ind = 0; ind < _ind; ind++) {
			//i = order[ind];
			...
			for (k = l2; k < h2; k++) {				
				dns_nnz2[k2] = dns_nnz[k];
				dns_nnz2[k2] = dns_nnz[k];
				dns_cols2[k2++] = dns_cols[k];
			}
			dns_row_starts2[ind+1] = k2;
		...
		for(int t4 = 0; t4 < 5; t4 ++)
		{
			...
			for (ind = 0; ind < _ind; ind++) {
				...
				//for (j = l1; j < h1; j++) {
					...
					l2 = dns_row_starts2[ind]; 
					h2 = dns_row_starts2[ind+1]; 

					for (k = l2; k < h2; k++) {
						tmpd = dns_nnz_r2[k] * dns_nnz2[k];
						d1 += tmpd;				
						d1 += dns_nnz2[k] * soln_below[dns_cols2[k]];
					}

				i = order[ind];
				if(d1 - soln_below[i] > diff2)
					diff2 = d1 - soln_below[i];
				if(diff2 < 0) printf(" !-! ");
				soln_below[i] = (row_starts[i+1] > row_starts[i]? d1:inf_vec[i] > 0 ? HUGE_VAL : 0);
			}
			if (diff2 < term_crit_param)
				break;
		}
	...
	{
		if(row_starts[i+1] > row_starts[i])
			soln_above[i] = soln_below[i] * 1.1 + .001;
		else
			soln_above[i] = soln_below[i] * 1.1;
		if(soln_below[i] > 0)
			s_q++;
		tkd[i] = false;
	}
	...
	{	tt = 0;
		...
		for (ind = 0; ind < _ind; ind++) {
			...
			i = order[ind];
			if(soln_above[i] - d3 > diff)
				diff = soln_above[i] - d3;

			//soln_below[i] = (h1 > l1) ? d1 : inf_vec[i] > 0 ? HUGE_VAL : 0;
			if(d3 < soln_above[i] && tkd[i] == false)
			{
				tkd[i] = true;
				s_q--;
			}
		...
		if(s_q == 0){
			printf("\nCorrectness of the approximated above vector satisfied after %d iterations.\n", iters - tmp_itr);
			s_q = -1;
		}
		// do matrix multiplication and min/max (from above)
		h1 = h2 = h2_r = 0;
...
//			PS_PrintToMainLog(env, "Max %sdiff between upper and lower bound on convergence: %G", measure.isRelative()?"relative ":"", diff);
	...
	printf("\nSecond round (updates from above) finished after %d iters.", iters);
	...
	{	tt = 0;
		...
		for (ind = 0; ind < _ind; ind++) {
			...
			soln_below[i] = (h1 > l1) ? d1 : inf_vec[i] > 0 ? HUGE_VAL : 0;
			soln_above[i] = (h1 > l1) ? d3 : inf_vec[i] > 0 ? HUGE_VAL : 0;
			//printf("\n%f", soln_above[i] - soln_below[i]);

	//		if(iters == 565)printf("\n %d , %f   , %f", i, dns_sr_vec[i], soln_above[i] - soln_below[i]);
			if(diff < soln_above[i] - soln_below[i])
