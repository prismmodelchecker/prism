...
JNIEXPORT jlong __jlongpointer JNICALL Java_sparse_PrismSparse_PS_1NondetReachRewardAsynchupperSCC
(
JNIEnv *env,
jclass cls,
...
)
{
	...
	double d1, d2, d3, d4, x, sup_norm, kb, kbt;
	bool done, first;
	int *scc_state;
	int *scc_start;
	int *scc_end;
	...
	printf("\nTime for pre-computation: %f \n", (util_cpu_time() - start2)/1000.);
		
	start2 = util_cpu_time();
	ticks_5 = current_scc_5 = 0;
	top_5 = -1;	
	...
	for(i = 0; i < n; i++)
	{
		...
		for(j = row_starts[i]; j < row_starts[i + 1]; j++)
			for(k = choice_starts[j]; k < choice_starts[j + 1]; k++)	
				g_5[i].push_back(cols[k]);			
	}
	for(i = 0; i < n; i++)
		if(scc_5[i] == -2)
			tarjan_5(i);

	number_of_sccs = current_scc_5;
	...
	for(mm = 0; mm < number_of_sccs; mm++)
		scc_freq[mm] = scc_start[mm];
	for(i = 0; i < _ind; i++)
	{
		j = scc_5[order[i]];
		queue[scc_freq[j]++] = order[i];
	}
	
	for(mm = 0; mm < number_of_sccs && iters < max_iters; mm++)
	{
		low = scc_start[mm];
		hi = scc_start[mm + 1];
		...
		if(hi - low < 2)
		{
			...
			while (!done && iters < max_iters) 
			{
				...
				i = scc_state[low];
				{
					...
					soln_above[i] = (h1 > l1 ? d1 : inf_vec[i] > 0 ? HUGE_VAL : 0);
		...
		dns_row_starts[0] = 0;
		m1 = m2 = m3 = m_r = 0;
		//for(ind = 0; ind < hi-low; ind++)
		for(ind = low; ind < hi; ind++)
		{
			i = queue[ind];
			//i = scc_state[ind];
		...
		while (!done && iters < max_iters) 
		{
			...
			for (ind = low; ind < hi; ind++) {
				...
				first = true; // (because we also remember 'first')
				 { l1 = dns_row_starts[ind-low]; h1 = dns_row_starts[ind-low+1]; }
				for (j = l1; j < h1; j++) {
					d2 = dns_sr_vec[ind-low];
					//d4 = dns_sr_vec[ind-low];
					{ l2 = dns_choice_starts[j]; h2 = dns_choice_starts[j+1]; }

					for (k = l2; k < h2; k++) {
						tmpd = dns_nnz_r[k] * dns_nnz[k];
						d2 += tmpd;				
						d2 += dns_nnz[k] * soln_above[dns_cols[k]];
						//d4 += tmpd;
		...
		//		if(iters == 565)printf("\n %d , %f   , %f", i, dns_sr_vec[i], soln_above[i] - soln_below[i]);
				if(d1 - soln_above[i] > diff)
					diff = d1 - soln_above[i];
			...
			// print occasional status update
			if ((util_cpu_time() - start3) > UPDATE_DELAY*2) {
				PS_PrintToMainLog(env, "Iteration %d: max %sdiff=%f", iters, measure.isRelative()?"relative ":"", diff);
				PS_PrintToMainLog(env, ", %.2f sec so far\n", ((double)(util_cpu_time() - start2)/1000));
				start3 = util_cpu_time();
			}
			...
			for (ind = low; ind < hi; ind++) {
				...
				for (j = l1; j < h1; j++) {
					...
					for (k = l2; k < h2; k++) {
						...
						d4 += tmpd;
						d4 += dns_nnz[k] * soln_below[dns_cols[k]];
					}
					//if (first || (min&&(d2<d1)) || (!min&&(d2>d1))) {
					//	d1 = d2;					
					//}
				...
				if(soln_below[i] - d3 > diff)
					diff = soln_below[i] - d3;

				soln_below[i] = (h1 > l1) ? d3 : inf_vec[i] > 0 ? HUGE_VAL : 0;
			}
			if (diff < term_crit_param) 
			{
				ss0 = util_cpu_time();
