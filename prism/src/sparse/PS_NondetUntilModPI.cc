...
{	
	...
	{	 
		...
		localitr = 0;
		while(!done && localitr < (iters<400?40:100))
		{
			iters++;
			localitr++;
			sup_norm = 0;
		...
		total_mults += one_iter_mults * localitr;
		total_updates += maybe_states * localitr;
		one_iter_mults = 0;
 		iters++;
		sup_norm = 0.0;
		...
		for(i = 0; i < n; i++) 
		{
			...
			soln[i] = d1;
			total_updates++;
			total_mults += choice_starts[row_starts[i+1]] - choice_starts[row_starts[i]];
