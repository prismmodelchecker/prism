...
JNIEXPORT jlong __jlongpointer JNICALL Java_sparse_PrismSparse_PS_1NondetUntilImprovedTopologicalMPI
(
JNIEnv *env,
jclass cls,
...
)
{
	...
	int M0, M1, M2, M3, top;
	int *uf_cols, *useful_states, *uf_choice_strt, *mapping;
	double *uf_nnz;
	...
	for(i = 0; i < n; i++)
	{
		...
		for(j = row_starts[i]; j < row_starts[i + 1]; j++)
			for(k = choice_starts[j]; k < choice_starts[j + 1]; k++)	
				g_4[i].push_back(cols[k]);			
	}
	for(i = 0; i < n; i++)
		if(scc_4[i] == -2)
			tarjan_4(i);
	number_of_sccs = current_scc_4;
	...
	{ttt = iters;
		...
		else
		{
			...
			for(m = low; m < hi; m++)
			{
				...
				for(j = row_starts[i]; j < row_starts[i+1]; j++)
					...
					{
						dest = cols[k];
						if(scc_4[dest] != ind && state_selected[i] == false)
						{
							state_order[left++] = i;
							state_selected[i] = true;
						}
		...
		terminate = done = false;
		while (!terminate && iters < max_iters) {
			iters++;
		// do matrix multiplication and min/max
			...
			for(m = low; m < hi; m++)
			{
				...
				if(dirac_group[i] == i)
				{
					...
					for(j = 0; j < choice_starts[1 + adv_starts[i]] - choice_starts[adv_starts[i]]; j++)
					{
						...
						if(scc_4[cols[M2 + j]] == ind && dirac_group[cols[M2 + j]] >= 0)
			...
			localitr = 0;
			while(!done && localitr < 150 && hi - low > 1)
			{
				localitr++;
				sup_norm = 0;
				...
				if (sup_norm < term_crit_param && localitr > (hi-low>10?10:1)) 
					done = true;					
			}
			iters += localitr;
