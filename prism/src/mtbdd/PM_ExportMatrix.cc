...
static void export_rec(DdNode *dd, DdNode **rvars, DdNode **cvars, int num_vars, int level, ODDNode *row, ODDNode *col, int64_t r, int64_t c)
{
	...
	if (level == num_vars) {
		switch (export_type) {
		case EXPORT_PLAIN: export_string("%" PRId64 " %" PRId64 " %.17g\n", r, c, Cudd_V(dd)); break;
		case EXPORT_MATLAB: export_string("%s(%" PRId64 ",%" PRId64 ")=%.17g;\n", export_name, r+1, c+1, Cudd_V(dd)); break;
		case EXPORT_DOT: case EXPORT_DOT_STATES: export_string("%" PRId64 " -> %" PRId64 " [ label=\"%.17g\" ];\n", r, c, Cudd_V(dd)); break;
		}
		return;
	}
