 ...
 * 
 * The following utility method is used for generating (and exporting) a single path satsifying certain criteria:
 * 
 * <UL>
...
public class SimulatorEngine
{
	 ...
	 *             If there was a problem with the deallocation, an exception is thrown.
	 */
	...
	/**
	 * Cleans up all c++ structures including the loaded model, loaded properties, the current path and any sampling
	...
	/**
	 * Notifies the simulator engine of a PRISM parsed ModulesFile. Loads in the model stored in modulesFile with the
	 * constants stored in constants. It also initialises the state space with the appropriate number of variables. If
	 * there are any problems with loading the model, is called before a SimulatorException is thrown.
	 */
	private void loadModulesFile(ModulesFile modulesFile, Values constants) throws SimulatorException
	{
		...
		if (modulesFile.getSystemDefn() != null) {
			throw new SimulatorException(
					"Sorry - the simulator does not currently handle the system...endsystem construct");
		}
		...
		try {
			...
			// SETUP MODEL
			int type = modulesFile.getModelType();
			...
			result = allocateModel(type, countCommands, numRewardStructs, numStateRewards, numTransitionRewards,
					modulesFile.getNumModules(), acind);
			if (result == ERROR) {
				throw new SimulatorException(getLastErrorMessage());
			}
			...
			for (int i = 0; i < modulesFile.getNumModules(); i++) {
				...
				for (int j = 0; j < m.getNumCommands(); j++) {
					...
					int actionIndex = (synch.equals("")) ? -1 // if no synch
							: ((Integer) (actionIndices.get(synch))).intValue(); // otherwise, use the synch

					int moduleIndex = ((Integer) (moduleIndices.get(m.getName()))).intValue();

					// Create the guard expression
					long guardPointer = c.getGuard().toSimulator(this);
					if (guardPointer == NULL) {
						throw new SimulatorException("Problem with loading model into simulator: null guardPointer in "
								+ m.getName());
					}
					...
					if (commandPointer == NULL) {
						throw new SimulatorException(
								"Problem with loading model into simulator: null commandPointer in " + m.getName());
					}
					...
					for (int k = 0; k < numUpdates; k++) {
						...
						if (probPointer == NULL) {
							throw new SimulatorException(
									"Problem with loading model into simulator: null probPointer in " + m.getName());
						}
						...
						if (updatePointer == NULL) {
							throw new SimulatorException(
									"Problem with loading model into simulator: null updatePointer in " + m.getName());
						}
						...
						for (int ii = 0; ii < numAssignments; ii++) {
							...
							if (rhsPointer == NULL) {
								throw new SimulatorException("Problem with loading model into simulator: null rhs in "
										+ m.getName());
							}
							...
							if (assign == NULL) {
								throw new SimulatorException(
										"Problem with loading model into simulator: null assignment in " + m.getName());
							}
						}
					}
			...
			for (int i = 0; i < numRewardStructs; i++) {
				...
				for (int j = 0; j < rewards.getNumItems(); j++) {
					...
					if (rewardPointer == NULL)
						throw new SimulatorException("Problem with loading model into simulator: null reward, "
								+ reward.toString());
					if (reward.isTransitionReward())
	...
	/**
	 * Model loading helper method Sets up the c++ engine state space variable table to have the given number of
	 * variables. Returns ERROR if there is a problem.
	 */
	private static native int allocateStateSpace(int numberOfVariables);

	/**
	 * Allocates space for a new model in the c++ engine according to the given parameters.
	 */
	...
	/**
	 * Model loading helper method Adds the CCommand object stored at the location of commandPointer to the c++ engine
	 * transition table. Returns ERROR if there is a problem
	 */
	private static native int setupAddTransition(long commandPointer);

	/**
	 * Model loading helper method Adds the CStateRewards object stored at the location of rewardPointer to the ith
	 * state rewards table. Returns ERROR if there is a problem
	 */
	private static native int setupAddStateReward(int i, long rewardPointer);

	/**
	 * Model loading helper method Adds the CTransitionRewards object stored at the location of rewardPointer to the ith
	 * transition rewards table. Returns ERROR if there is a problem
	 */
	 ...
	 *            the variable string.
	 * @throws SimulatorException
	 *             if the string was not found or is invalid.
	...
	public int getIndexOfVar(String name) throws SimulatorException
	{
		if (varIndices.containsKey(name))
			...
			throw new SimulatorException("Cannot get variable index, name: " + name + " does not exist");
	}
	 ...
	 * @throws SimulatorException
	 *             if the string was not found or invalid.
	...
	public int getIndexOfAction(String name) throws SimulatorException
	{
		if (actionIndices.containsKey(name))
			...
			throw new SimulatorException("Cannot get action index, name: " + name + " does not exist");
	}
	...
	private void loadInitialState(Values initialState) throws SimulatorException
	{
		...
		try {
			...
			for (i = 0; i < initialState.getNumValues(); i++) {
				...
				if (initialState.getType(i) instanceof TypeBool)
					value = (initialState.getBooleanValue(i)) ? 1 : 0;
				else if (initialState.getType(i) instanceof TypeInt)
					value = initialState.getIntValue(i);
				else
					throw new SimulatorException("Invalid type for variable " + initialState.getName(i));
	...
	/**
	 * Starts a new path, loads the model data structure with its constants defined constants. Prerequisites: CONSTANTS
	 * MUST BE DEFINED BEFORE CALLING THIS METHOD
	 * 
	 * @param modulesFile
	 *            The <tt>ModulesFile</tt> object to be converted into the simulator data structure for a PRISM model.
	 * @param propertiesFile
	 *            The <tt>PropertiesFile</tt> object that any pctl path properties belong to.
	 * @param initialState
	 *            The intial state for the new path.
	 * @throws SimulatorException
	 *             If anything goes wrong with the model loading, or the initialisation of the new path.
	 */
	public void startNewPath(ModulesFile modulesFile, PropertiesFile propertiesFile, Values initialState)
			throws SimulatorException
	{
		deallocateEngine();
		...
		if (propertiesFile == null)
			propertiesFile = new PropertiesFile(modulesFile);
	 ...
	 * 
	 * @param initialState
	 *            the initial state to be set into the simulator engine.
	 * @throws SimulatorException
	 *             if there is any problem with setting this state.
	 */
		...
		try {
			...
			for (int i = 0; i < initialState.getNumValues(); i++) {
				...
				if (initialState.getType(i) instanceof TypeBool)
					...
					throw new SimulatorException("Cannot start new path: Invalid type for variable "
							+ initialState.getName(i));
	...
	/**
	 * Allocates an execution path in memory for the purposes of model exploration. This should not be used if wishing
	 * to do sampling.
	 */
	 ...
	 *            the variable index
	 * @throws SimulatorException
	 *             if the variable index is out of range.
	...
	public String getVariableName(int i) throws SimulatorException
	{
		if (varNames != null && i < varNames.length && i >= 0) {
			...
			throw new SimulatorException("Variable out of range");
	}
	 ...
	 * @throws SimulatorException
	 *             if the variable index is out of range.
	 * @return a subclass of Type denoting the type of the indexed variable.
	 */
	public Type getVariableType(int i) throws SimulatorException
	{
	 ...
	 * 
	 * @param file
	 *            the file to which the path should be exported to (mainLog if null).
	 * @throws SimulatorException
	 *             if there is a problem with writing the file.
	 */
	 ...
	 * @param vars
	 *            only print these vars (and steps which change them) (ignore if null)
	 * @throws SimulatorException
	 *             if there is a problem with writing the file.
	 */
	 ...
	 * 
	 * @param filename
	 *            the PrismLog to which the path should be exported to
	 * @param colSep
	 *            string used to separate columns in display
	 * @param vars
	 *            only print these vars (and steps which change them) (ignore if null)
	 ...
	 *             if there is a problem with writing the file.
	 */
	public void exportPathToLog(PrismLog log, boolean timeCumul, String colSep, ArrayList vars)
			throws SimulatorException
	{
		int i, j, n, nv, nr;
		double d, t;
		boolean stochastic = (modulesFile.getModelType() == ModulesFile.STOCHASTIC);
		boolean changed;
		...
		if (vars != null) {
			...
			for (i = 0; i < varsNum; i++)
				varsIndices[i] = ((Integer) vars.get(i)).intValue();
		}
		...
		if (vars == null)
			...
			for (j = 0; j < varsNum; j++)
				log.print(colSep + varNames[varsIndices[j]]);
		if (stochastic)
		...
		for (i = 0; i < n; i++) {
			...
			if (vars != null && i > 0) {
				changed = false;
				for (j = 0; j < varsNum; j++) {
					if (getPathData(varsIndices[j], i) != getPathData(varsIndices[j], i - 1))
						changed = true;
				}
				if (!changed) {
			...
			if (vars == null) {
				for (j = 0; j < nv; j++) {
					log.print(colSep);
					if (varTypes[j] instanceof TypeBool)
						log.print((getPathData(j, i) == 0) ? "false" : "true");
					else
				...
				for (j = 0; j < varsNum; j++) {
					log.print(colSep);
					if (varTypes[j] instanceof TypeBool)
						log.print((getPathData(varsIndices[j], i) == 0) ? "false" : "true");
					else
						log.print(getPathData(varsIndices[j], i));
				}
			}
			if (stochastic) {
				d = (i < n - 1) ? getTimeSpentInPathState(i) : 0.0;
	...
	/**
	 * Ask c++ engine to make the update at the given index. Returns OUTOFRANGE (=-1) if the index is out of range.
	 */
	...
	/**
	 * Returns the action label of the update at the given index.
	 * 
	 * @param i
	 *            the index of the update of interest.
	 * @return the action label of the update at the given index.
	 */
	...
	public String getAssignmentDescriptionOfUpdate(int index)
	{
		...
		for (int i = 0; i < numAssigns; i++) {
			...
			if (varTypes[var] instanceof TypeBool)
				if (value == 0)
	...
	public long addExpressionReward(ExpressionReward expr)
	{
		...
		try {
			...
			case ExpressionTemporal.R_F:
				if (!(exprTemp.getOperand2().getType() instanceof TypeBool))
					return -1;
	...
	public long addExpressionProb(ExpressionProb expr)
	{
		...
		try {
			...
			case ExpressionTemporal.P_U:
				if ((exprTemp.getOperand1().getType() instanceof TypeBool))
					return -1;
