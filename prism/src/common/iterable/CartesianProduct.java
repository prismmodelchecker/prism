...
public class CartesianProduct
{
	...
	{
		int arity = FunctionalIterator.extend(iterables).count();
		assert FunctionalIterator.extend(iterables).count() <= arity;
	...
	protected static <T> FunctionalIterable<T[]> mutableOf(Iterator<Iterable<? extends T>> iterables, T[] tuple, int pos)
	{
		...
		FunctionalIterable<? extends T> elements = FunctionalIterable.extend(iterables.next());
